...
namespace RecipeConfigurationApp.Validation
{
    class VacuumValidation
    {
        ...
        public void validateSetPoint(string value)
        {
            ...
            if (result)
            {
                if (number < -1 || number > 1)
                    throw new ArgumentOutOfRangeException("Setpoint", value, "Valor fora dos Limites especificados (>= -1.00 ou <= 1.00)");
            }
            else
            {
        ...
        public void validateTaxa(string value)
        {
            ...
            if (result)
            {
                if (number < -10 || number > 10)
                    throw new ArgumentOutOfRangeException("Taxa", value, "Valor fora dos Limites especificados (>= -10.00 ou  <= 10.00)");
                if (number == 0)
        ...
        public void validateTolerancia(string value)
        {
            ...
            {
                if (number < -10 || number > 10)
                    throw new ArgumentOutOfRangeException("Tolerancia", value, "Valor fora dos Limites especificados (>= -10.00 ou <= 10.00)");
            }
            else
            {
                throw new FormatException("Tipo de dado não corresponde ao tipo de dado do campo");
